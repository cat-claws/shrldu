"""Natural-language GUI demo backed by a preplanned Ollama agent."""
...
def demo(model: str = DEFAULT_MODEL, host: str = 'http://127.0.0.1:11434',
    ...
    """Run the GUI demo with a plan-first Ollama agent."""
    pygame.init()

    env = ShrdluBlocksEnv()
    agent = PreplannedOllamaShrdluAgent(
        env,
        model=model,
        host=host,
        max_steps=max_steps,
    ...
    viewer = Viewer(
        screen,
        "SHRDLU Blocks Preplanned Ollama Demo",
