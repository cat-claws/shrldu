"""Natural-language GUI demo backed by a preplanned OpenAI-compatible agent."""
...
def demo(model: str = DEFAULT_OPENAI_MODEL,
         ...
         max_tokens: int = 512):
    """Run the GUI demo with a plan-first OpenAI-compatible agent."""
    pygame.init()

    env = ShrdluBlocksEnv()
    agent = PreplannedOpenAICompatibleShrdluAgent(
        env,
        model=model,
        base_url=base_url,
        api_key=api_key,
    ...
    viewer = Viewer(
        screen,
        "SHRDLU Blocks Preplanned OpenAI-Compatible Demo",
        callback=handle_text,
        initial_output=(
            'Type a natural-language instruction.\n'
            'This demo plans the full action sequence first, then executes it.\n'
