"""Natural-language GUI demo backed by a suffix predictive preplanned OpenAI-compatible agent."""
...
def demo(model: str = DEFAULT_OPENAI_MODEL,
    ...
    """Run the GUI demo with a suffix predictive preplanned OpenAI-compatible agent."""
    pygame.init()

    env = ShrdluBlocksEnv()
    agent = SuffixPredictivePreplannedOpenAICompatibleShrdluAgent(
        env,
        model=model,
        base_url=base_url,
        api_key=api_key,
    ...
    viewer = Viewer(
        screen,
        "SHRDLU Blocks Suffix Predictive Preplanned OpenAI-Compatible Demo",
        callback=handle_text,
        initial_output=(
            'Type a natural-language instruction.\n'
            'This demo plans a full remaining suffix, predicts along it, replans from violations, then executes once.\n'
